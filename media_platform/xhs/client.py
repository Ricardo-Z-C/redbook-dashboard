...
from .exception import DataFetchError, IPBlockError, NonRetryableDataFetchError, NoteNotFoundError
...
def is_non_retryable_error_msg(err_msg: str) -> bool:
    return any(keyword in err_msg for keyword in NON_RETRYABLE_ERROR_KEYWORDS)
...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    @retry(
        ...
        wait=wait_fixed(1),
        retry=retry_if_not_exception_type((NoteNotFoundError, NonRetryableDataFetchError)),
    )
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code == 471 or response.status_code == 461:
            ...
            raise NonRetryableDataFetchError(msg)
        ...
        if data["success"]:
            ...
            if is_non_retryable_error_msg(err_msg):
                utils.logger.warning(
                    f"[XiaoHongShuClient.request] non-retryable error detected, stop retries: {err_msg}"
                )
                raise NonRetryableDataFetchError(err_msg)
