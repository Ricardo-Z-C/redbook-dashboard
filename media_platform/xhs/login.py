...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_cookies(self):
        ...
        if not cookie_dict:
            utils.logger.warning("[XiaoHongShuLogin.login_by_cookies] Empty cookie string, fallback to qrcode may be required.")
            return
        ...
        if cookies_payload:
            await self.browser_context.add_cookies(cookies_payload)
            utils.logger.info(
                f"[XiaoHongShuLogin.login_by_cookies] Imported {len(cookies_payload)} cookies into browser context."
            )
