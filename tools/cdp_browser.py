...
class CDPBrowserManager:
    ...
    async def launch_and_connect(
        ...
        """
        try:
            # Check if we can connect to the configured port directly (User manually started browser)
            if await self._test_cdp_connection(config.CDP_DEBUG_PORT):
                utils.logger.info(f"[CDPBrowserManager] Detected existing browser on port {config.CDP_DEBUG_PORT}, connecting...")
