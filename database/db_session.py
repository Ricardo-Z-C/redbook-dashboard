...
def _build_mysql_async_url(with_database=True):
    return URL.create(
        drivername="mysql+asyncmy",
        username=str(mysql_db_config["user"]),
        password=str(mysql_db_config["password"]),
        host=str(mysql_db_config["host"]),
        port=int(mysql_db_config["port"]),
        database=str(mysql_db_config["db_name"]) if with_database else None,
    )
...
async def create_database_if_not_exists(db_type: str):
    if db_type == "mysql" or db_type == "db":
        ...
        server_url = _build_mysql_async_url(with_database=False)
...
def get_async_engine(db_type: str = None):
    ...
    if db_type == "sqlite":
        ...
        db_url = _build_mysql_async_url(with_database=True)
    elif db_type == "postgres":
    ...
    if engine:
        async with engine.begin() as conn:
            try:
                ...
                # Some managed competition databases grant table DDL but deny INDEX privileges.
                # Fallback: create tables without secondary indexes so core read/write can proceed.
                if db_type in {"mysql", "db"} and "INDEX command denied" in str(exc):
